...
const PathStop = (props) => {
  ...
  return (
    ...
    >
      <div
        className={`h-[3.125rem] max-w-min flex justify-end relative`}
      >
        <div className={`h-0 w-0 absolute top-0 left-1/2 -translate-x-1/2 origin-bottom ${styles["bouncing-stop"]} ${styles["stop-post"]}`}>
        </div>
        <div className={`aspect-square h-10 bg-white rounded-full flex items-center absolute left-1/2 bottom-0 ${styles["stop-base"]}`}>
          <div className={`${styles["stop-post-shadow"]} ${styles["bouncing-stop-shadow"]}`}>
          </div>
        </div>
      </div>
      <p className="mt-4 text-center">{props.title}</p>
    </div>
  )
}
